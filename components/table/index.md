...
| fixHeader | 表头固定，给定需要固定高度的具体数值，当超出该值时，展示滚动条 | `boolean` &#124; `number` &#124; `string` | `false` | |
...
| checkType | 当行可选择时，定义选择的类型: 单选 `"radio"` , 复选 `"checkbox"`, 不可选`"none"`（设置后表格首列会自动渲染checkbox/radio，无需手动定义选择列） | `"checkbox"` &#124; `"radio"` &#124; `"none"` | `"checkbox"` | |
| checkedKeys | 通过`key`数组来指定哪些行被选中；需要双向绑定（Vue用`v-model:checkedKeys`或`:checkedKeys.sync`），否则选中状态不会更新 | `TableRowKey[]` | `[]` | |
| rowKey | 设置每行的key；只接受函数类型 `(value: T, index: number) => TableRowKey`，不得传字符串 | `(value: T, index: number) => TableRowKey` | `(value: T, index: number) => index` | |
...
| pagination | 是否开启内置分页（按 data 内部切片，需传入完整数据） | `boolean` &#124; `PaginationProps` | `false` | |
| fixFooter | `table`给定需要固定高度时，自定义footer固定 | `boolean` | `false` | |
| load | 指定异步加载节点数据的函数，该函数通过`Promise`返回数组来添加子节点数据 | <code>(node: any) => Promise<void> &#124; void</code> | `undefined` | 3.4.3 |
| spreadArrowIndex | 指定树形表格展开Icon的所在列，默认在第一列 | `number` | `0` | 3.4.5 |
| virtual | 是否开启虚拟列表 | `boolean` | `false` | 3.5.0 |
...
| hideGroupHeader | 是否隐藏group下拉菜单中header区域 | `boolean` | `false` | 3.4.2 |
...
| hidden | 是否隐藏当前列，仅为不可见，不影响`exportTable`导出表格 | `boolean` | `false` | 3.3.3 |
| ellipsis | 是否开启超长省略 | `boolean` | `false` | 3.4.3 |
...
| --- | --- | --- |
| title | 自定义复杂表头内容 | - |
| template | 自定义单元格内容。注意：参数为数组解构形式 `[data, index]`，而非对象解构 | `([data: T, index: number]) => Children` |
| default | 与`template`扩展点等价，为了方便vue中使用的别名。注意：参数为数组解构形式 `[data, index]`，而非对象解构 | `([data: T, index: number]) => Children` |
