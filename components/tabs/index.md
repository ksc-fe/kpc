...
> `Tabs` 设计用于页面导航，仅规定了头部的样式和交互。`Tab` 内可以嵌套内容，用于自定义选项卡标签的展示；嵌套内容不会作为可切换的面板内容。
> 如果需要切换 `Tab` 从而切换内容的效果，请根据 `Tabs` 绑定的值，配合 `v-if`（Vue）或条件渲染（React）在 `Tabs` 外部手动控制内容的展示。详见示例。
