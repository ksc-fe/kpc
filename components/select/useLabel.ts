import {Children, VNodeComponentClass, VNode, useInstance, watch, createRef} from 'intact';
import {isNullOrUndefined, isStringOrNumber, EMPTY_OBJ} from 'intact-shared';
import {findChildren, isEmptyString, ValidVNode} from '../utils';
    ...
    function findLabel(children: Children, value: any) {
        ...
        const loop = (children: Children) => {
            return findChildren(children, (vNode) => {
                ...
                } else if ((vNode as VNodeComponentClass).tag === OptionGroup) {
                    if (loop(((vNode as VNodeComponentClass).props || EMPTY_OBJ).children)) {
